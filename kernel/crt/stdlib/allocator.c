...
static AllocatorEntry *FindFreeEntry(size_t Size) {
    ...
    uint64_t EntryPage = (uint64_t)pmm_alloc((Size + sizeof(AllocatorEntry) + 4095) >> 12);
    if (!EntryPage) {
        return NULL;
    }

    Entry = (AllocatorEntry *)(EntryPage + pmm_hhdm_req.response->offset);
    Size = ((Size + sizeof(AllocatorEntry) + 4095) & ~0xFFF) - sizeof(AllocatorEntry);
